...
public class JWTFilter extends BasicHttpAuthenticationFilter {
  ...
  private boolean verificationPassAnnotation(
      ...
      throws Exception {
    for (Constant.URLInfo info : Constant.METHOD_URL_SET) {
      if (info.getUrl().equals(httpServletRequest.getRequestURI())
          && (info.getMethodType().equals(httpServletRequest.getMethod()))) {
      ...
      if (StringUtils.countMatches(info.getUrl(), "{") > 0
          && (info.getMethodType().equals(httpServletRequest.getMethod())
              || RequestMapping.class.getName().equals((info.getMethodType())))) {
        if (isSameUrl(info.getUrl(), httpServletRequest.getRequestURI())) {
          Constant.isPass = true;
          if (ComUtil.isEmpty(authorization)) {
            httpServletRequest.setAttribute("currentUser", new Admin());
            return true;
